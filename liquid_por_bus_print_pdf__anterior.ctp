...
$pdf->MultiCell(22, 0, 'B/VIAJE', 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(18, 0, 'IMPORTE', 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(8, 0, 'N°', 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(22, 0, 'B/VIAJE', 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(18, 0, 'IMPORTE', 1, 'C', 1, 0, '', '', true);
...
foreach ($fndSales as $key => $value) {
	if ($key<38){
		...
		$pdf->SetFont('helvetica', 'N', 8);
		$pdf->setCellPaddings(1, 1, 1, 1);
		$pdf->MultiCell(8, 0, $num, 1, 'C', 1, 0, '', '', true);
		$pdf->MultiCell(22, 0, $value['sales']['serie'].'-'.str_pad($value['sales']['number'], '7', STR_PAD_LEFT, '0'), 1, 'C', 1, 0, '', '', true);
		$pdf->MultiCell(18, 0, number_format($value['sales']['price'], 2), 1, 'C', 1, 0, '', '', true);	
...
$cordYln	= 5.527;	//cordenada Y salto de linea
$cordXfe	= 107;		//cordenada X factura encomienda
$pdf->SetXY($cordXfe, $cordYfe);
$pdf->SetFont('helvetica', 'N', 10);
$pdf->SetFillColor(255, 255, 255);
$pdf->MultiCell(0, 0, 'FACTURAS ENCOMIENDAS', 0, 'L', 1, 0, '', '', true);

$pdf->Ln();
$pdf->SetXY($cordXfe, $cordYfe+$cordYln);
$pdf->SetFont('helvetica', 'B', 8);
$pdf->setCellPaddings(1, 1, 1, 1);
$pdf->MultiCell(22, 0, 'N°/FACTURA', 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(22, 0, 'IMPORTE', 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(22, 0, 'N°/FACTURA', 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(22, 0, 'IMPORTE', 1, 'C', 1, 0, '', '', true);
...
foreach ($fndCommend as $key => $value) {
	if ($key<5) {
		...
		$pdf->MultiCell(22, 0, $value['Commends']['serie'].'-'.str_pad($value['Commends']['number'], '7', STR_PAD_LEFT, '0'), 1, 'C', 1, 0, '', '', true);
		$pdf->MultiCell(22, 0, number_format($value['Commends']['total'], 2), 1, 'C', 1, 0, '', '', true);
		$cordY 			+= $cordYln;
		$sumComFac_1	+= $value['Commends']['total'];
	}
}
...
foreach ($fndCommend as $key => $value) {
	if ($key>=5 AND $key<10) {
		...
		$pdf->MultiCell(22, 0, number_format($value['Commends']['total'], 2), 1, 'C', 1, 0, '', '', true);
		$cordY 			+= $cordYln;
		$sumComFac_2	+= $value['Commends']['total'];
	}
}

//---totales FACTURA
$pdf->Ln();
$pdf->SetXY($cordXfe-44, $cordYfe+38);
$pdf->SetFont('helvetica', 'B', 8);
...
$sumaTotalComFac	= $sumComFac_1+$sumComFac_2;
$pdf->SetXY($cordXfe, $cordYfe+43.63);
$pdf->MultiCell(22, 0, 'TOTAL', 0, 'C', 1, 0, '', '', true);
$pdf->MultiCell(22, 0, number_format($sumaTotalComFac, 2), 1, 'C', 1, 0, '', '', true);
//-------------------

//b. BOLETAS ENCOMIENDAS

$cordYfe	= 67;		//cordenada Y factura encomienda
$cordYln	= 5.527;	//cordenada Y salto de linea
$cordXfe	= 107;		//cordenada X factura encomienda
$pdf->SetXY($cordXfe, $cordYfe);
$pdf->SetFont('helvetica', 'N', 10);
$pdf->SetFillColor(255, 255, 255);
$pdf->MultiCell(0, 0, 'BOLETAS ENCOMIENDAS', 0, 'L', 1, 0, '', '', true);
...
//menores a 5 documentos
$cordY	= $cordYfe+($cordYln*2);
$contItems	= 0;		//contar items factura;
$sumComBol_1	= 0;
foreach ($fndCommendBol as $key => $value) {
	if ($key<17) {
		$pdf->SetXY($cordXfe,$cordY);		
		$pdf->MultiCell(22, 0, $value['Commends']['serie'].'-'.str_pad($value['Commends']['number'], '7', STR_PAD_LEFT, '0'), 1, 'C', 1, 0, '', '', true);
		$pdf->MultiCell(22, 0, number_format($value['Commends']['total'], 2), 1, 'C', 1, 0, '', '', true);
		$cordY 			+= $cordYln;
		$sumComBol_1	+= $value['Commends']['total'];
	}
}
...
$sumComBol_2	= 0;
foreach ($fndCommendBol as $key => $value) {
	if ($key>=17 AND $key<34) {
		$pdf->SetXY($cordXfe,$cordY);		
		$pdf->MultiCell(22, 0, $value['Commends']['serie'].'-'.str_pad($value['Commends']['number'], '7', STR_PAD_LEFT, '0'), 1, 'C', 1, 0, '', '', true);
		$pdf->MultiCell(22, 0, number_format($value['Commends']['total'], 2), 1, 'C', 1, 0, '', '', true);
		$cordY 			+= $cordYln;
		$sumComBol_2	+= $value['Commends']['total'];
	}
}

//---totales BOLETA
$pdf->Ln();
$pdf->SetXY($cordXfe-44, $cordYfe+105);	//38
$pdf->SetFont('helvetica', 'B', 8);
$pdf->MultiCell(22, 0, 'SUBTOTAL', 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(22, 0, number_format($sumComBol_1, 2), 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(22, 0, 'SUBTOTAL', 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(22, 0, number_format($sumComBol_2, 2), 1, 'C', 1, 0, '', '', true);

$sumaTotalComBol	= $sumComBol_1+$sumComBol_2;
$pdf->SetXY($cordXfe, $cordYfe+110.63);
$pdf->MultiCell(22, 0, 'TOTAL', 0, 'C', 1, 0, '', '', true);
$pdf->MultiCell(22, 0, number_format($sumaTotalComBol, 2), 1, 'C', 1, 0, '', '', true);
//-------------------

//c. EXCESOS
$cordYfe	= 185;		//cordenada Y factura encomienda
$cordYln	= 5.527;	//cordenada Y salto de linea
$cordXfe	= 107;		//cordenada X factura encomienda
$pdf->SetXY($cordXfe, $cordYfe);
$pdf->SetFont('helvetica', 'N', 10);
...
foreach ($fndIncomes as $key => $value) {
	...
	if ($key<5) {
		$pdf->SetXY($cordXfe,$cordY);		
		$pdf->MultiCell(22, 0, str_pad($value['number'], '7', STR_PAD_LEFT, '0').' '.$incDocRes, 1, 'L', 1, 0, '', '', true);
		$pdf->MultiCell(22, 0, number_format($value['total'], 2), 1, 'C', 1, 0, '', '', true);
		$cordY 			+= $cordYln;
...
//sumar ingresos
$ingresos 	= $sumSalesTotal+$sumaTotalComFac+$sumaTotalComBol+$sumaTotalInc;
...
$pdf->MultiCell(28, 0, number_format($diferencia,2), 1, 'R', 1, 0, '', '', true);

$pdf->SetFont('helvetica', 'I', 7);
$pdf->Ln(8);
$pdf->MultiCell(0, 0, '* FECHA IMPRESION : '.date('d-m-Y g:i A').' - '.$first_name , 0, 'C', 1, 1, '', '', true);
