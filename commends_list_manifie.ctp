...
<br>
	<div class="col-sm-12">
		...
		<div class="col-sm-4">
				<?php echo $this->Ajax->form(['type' => 'post','options' => ['update'=>'filterdata','url' => ['controller' => 'sales','action' => 'commends_find_manifie_date'],'class'=>'form-inline','before'=>"$('.btn').attr('disabled','disabled');",'complete'=>"$('.btn').prop('disabled', false)"]]);
					echo $this->Form->input('date',['append' => '<i class="fa fa-calendar"></i>','id'=>'date','label'=>'','value'=>date('Y-m-d')]); 
					echo ' '.$this->Form->button('Consultar Fecha',array('id'=>'imprimir','class'=>'btn btn-blue'));
					echo ' '.$this->Html->link($this->Html->tag('span', '', array('class' => 'fa fa-pencil')).' &nbsp; Nuevo manifiesto ',['controller'=>'programations','action' => 'add2'], ['data-toggle'=>"modal",'data-target'=>"#myModal",'id'=>'boton','class'=>'btn btn-success','style'=>"margin-top:12px;margin-left:100px;",'escape' =>false]) ;
					echo $this->Form->end();
				?>
		</div>
		<div class="col-sm-4">.</div>
				...
				foreach ($datos as $value):
					$id=$value['programation']['id']; //programations id
						...
						echo '<tr>';
							...
							<td><?php echo $this->Ajax->link($this->Html->tag('span', '', array('class' => 'fa fa-user')),['controller' => 'programations', 'action' => 'commendDetailCrewsAdd', $id],['indicator' => 'cargando', 'data-toggle'=>"modal", 'data-target'=>"#myModal", 'class'=>'btn btn-primary btn-xs', 'escape'=>false]);?></td>
...
<iframe width="1200" height="300" src="" name="manif_iframe" allowfullscreen style="display:none;" ></iframe>
